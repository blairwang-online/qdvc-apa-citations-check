...
|---|---|---|---|
| `check_citations(file_path)` | Runs the full APA citation audit on the given file and prints a report. | `"paper.txt"` (a file containing body text with (Author, Year) citations and a References section) | *(printed)* the six-section APA CITATION AUDIT REPORT |
...
- `INTEXT_AUTHOR_SEPARATOR` — the expected author separator before the final author in an in-text citation: `"and"` for `(Smith and Jones 2026)` or `"&"` for `(Smith & Jones 2026)`.
- `REFLIST_AUTHOR_SEPARATOR` — the same, but for reference-list entries: `"and"` for `Smith A and Jones B (2026) ...` or `"&"` for `Smith A & Jones B (2026) ...`.
...
| Function | Description | Example Input | Example Output |
|---|---|---|---|
| `print_report(missing_in_references, unused_references, style_violations, order_violations, uses_comma_intext, intext_separator_violations=None, reference_separator_violations=None, intext_separator="and", reflist_separator="and")` | Prints the formatted six-section APA citation audit report to stdout. Returns nothing. The two separator-violation arguments and the separator labels are optional, so older five-argument calls still work (sections 5–6 then report "None"). | `missing_in_references = {"Nguyen, 2025"}`<br>`unused_references = ["Patel, R. (2021). Unused reference."]`<br>`style_violations = ["Jones 2024"]`<br>`order_violations = ["Jones, A. (2024). Another study."]`<br>`uses_comma_intext = True`<br>`intext_separator_violations = ["Smith & Jones 2020"]`<br>`reference_separator_violations = ["Brown C and Lee D (2019) ..."]`<br>`intext_separator = "and"`, `reflist_separator = "&"` | *(printed)*<br>`--- APA CITATION AUDIT REPORT ---`<br>`1. ... missing: - (Nguyen, 2025)`<br>`2. ... unused: - Patel, R. (2021). Unused reference.`<br>`3. ... style: - (Jones 2024)`<br>`4. ... order: - Jones, A. (2024). Another study.`<br>`5. ... intext separator: - (Smith & Jones 2020)`<br>`6. ... reflist separator: - Brown C and Lee D (2019) ...` |
